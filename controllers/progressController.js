    ...
    try {
       ...
       WHERE cl.id = $2`,
        ...
        const result = await pool.query(
            `INSERT INTO progreso_clases (usuario_id, clase_id, porcentaje_visto, completada)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (usuario_id, clase_id)
       DO UPDATE SET 
         porcentaje_visto = GREATEST(progreso_clases.porcentaje_visto, $3),
...
const getCourseProgress = async (req, res) => {
    try {
        ...
        const accesoCheck = await pool.query(
            `SELECT c.titulo, c.es_gratuito, i.estado_pago
       FROM cursos c
       LEFT JOIN inscripciones i ON c.id = i.curso_id AND i.usuario_id = $1
       WHERE c.id = $2 AND c.activo = true`,
        ...
        const resumenResult = await pool.query(
         ...
         COUNT(DISTINCT CASE WHEN pc.completada = true THEN m.id END) as modulos_con_progreso
       FROM clases cl
       JOIN modulos m ON cl.modulo_id = m.id
        ...
        const detalleResult = await pool.query(
         ...
         json_agg(
           json_build_object(
             ...
             'porcentaje_visto', COALESCE(pc.porcentaje_visto, 0),
       ...
       LEFT JOIN progreso_clases pc ON cl.id = pc.clase_id AND pc.usuario_id = $1
       WHERE m.curso_id = $2
       GROUP BY m.id, m.titulo, m.orden
       ORDER BY m.orden`,
